...
Key design constraint: The BoP button is a `SecureActionButtonTemplate` which means **Show(), Hide(), EnableMouse(), SetPoint(), SetSize() are all protected and cannot be called in combat** in TBC Classic. The addon works around this by keeping the button permanently shown and toggling visibility via `SetAlpha()` (which is not protected). This is intentional and must be preserved.

The addon has three logical layers:
- **SecureActionButton** (`BopNouarButton`) — always-visible macro button, alpha-toggled, executes `/cast [@Twistedrogue] Blessing of Protection`
